...
export function WorldIDVerification({ onVerified }: WorldIDVerificationProps) {
  ...
  // Efecto para manejar la secuencia automática de verificación
  useEffect(() => {
    if (!showQRDialog || verificationStage !== "qr") return

    // Después de 8 segundos, cambiar a estado de carga
    const loadingTimer = setTimeout(() => {
      setVerificationStage("loading")

      // Después de 4 segundos más, completar la verificación
      const completeTimer = setTimeout(() => {
        setVerificationStage("complete")

        // Pequeña pausa antes de cerrar el diálogo
        setTimeout(() => {
  ...
  return (
    <>
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          if (!open) {
            // Solo permitir cerrar el diálogo si no estamos en medio del proceso
